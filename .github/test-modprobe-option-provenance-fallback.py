...
def script_mutation(root):
    conf = root / "mutation-config"; conf.mkdir(); out = root / "mutation-output"
    (conf / "mutation.conf").write_text(
        "alias mutation-marker mod-loop-a\n"
        "install mutation-outer /bin/sh -c 'MODPROBE_OPTIONS=\"$MODPROBE_OPTIONS -q\"; "
        "export MODPROBE_OPTIONS; exec " + shlex.quote(new) + " -R mutation-marker > " + shlex.quote(str(out)) + "'\n"
    )
    result = run([new, '-C', str(conf), 'mutation-outer'])
    marker = out.read_text().splitlines().count('mod_loop_a') if out.exists() else 0
    return {'run': result, 'marker': marker}
...
with tempfile.TemporaryDirectory(prefix='kmod-provenance-') as td:
    ...
    mutation = script_mutation(root)
    mixed = mixed_versions(root)
    result = {'modprobe': new, 'cli': cli, 'base_stability': stable, 'script_mutation': mutation, 'mixed_versions': mixed}
    ...
    assert mutation['run']['status'] == 0 and mutation['marker'] == 1
